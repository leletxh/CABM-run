...
namespace 启动器
{
    public partial class dl : Form
    {
        ...
        private const string MIRROR_PREFIX = "https://wget.la/";
        ...
        private void dl_Load(object sender, EventArgs e)
        {
            ...
            if (launchCount >= 50)
            {
                update_log("检测到超过50次启动，询问是否开启彩蛋喵~");
                ...
                {
                    update_log("用户选择开启彩蛋，打开网页喵~");
                    OpenSpecialWebPage();
                }
                else
                {
                    update_log("你选择不开启彩蛋喵");
                }
        ...
        private int GetLaunchCountFromRegistry()
        {
            ...
            {
                update_log($"读取启动次数失败: {ex.Message}喵");
        ...
        private void IncrementLaunchCountInRegistry()
        {
            try
            {
                using (RegistryKey key = Registry.CurrentUser.CreateSubKey(REGISTRY_KEY_PATH))
                {
                    ...
                    update_log($"启动次数已增加到: {newCount}喵");
                }
            }
            catch (Exception ex)
            {
                update_log($"增加启动次数失败: {ex.Message}喵");
            }
        }
        ...
        private void ResetLaunchCountInRegistry()
        {
            try
            {
                using (RegistryKey key = Registry.CurrentUser.CreateSubKey(REGISTRY_KEY_PATH))
                {
                    ...
                    update_log("启动次数已重置为 0喵");
                }
            }
            catch (Exception ex)
            {
                update_log($"重置启动次数失败: {ex.Message}喵");
            }
        }
        ...
        private void OpenSpecialWebPage()
        {
            try
            {
                ...
                update_log($"尝试打开网页: {url}喵");
                System.Diagnostics.Process.Start(url);
                update_log("网页已打开喵");
            }
            catch (Exception ex)
            {
                update_log($"打开网页失败: {ex.Message}喵");
        ...
        private async void start_download()
        {
            Thread.Sleep(1000);
            update_log("才怪");
            update_log("南梁启动器（划掉）CABM启动器初始化成功");
            Thread.Sleep(1000);
            update_log("我要干嘛来着");
            Thread.Sleep(1000);
            update_log("开始深度思考喵~");
            ...
            update_log("我要思考什么来着？");
            Thread.Sleep(1000);
            update_log("奥对，是下载东西啊，真是的，我一定要让米忽悠给我升级");
            try
            {
                update_log("嗯");
                Thread.Sleep(1000);
                update_log("让我瞅瞅你安装了吗？");
                SetProgress(0.0f);
                ...
                else
                {
                    ...
                    Thread.Sleep(1000);
                    update_log("哦吼！");
                    update_log("我没瞅到，算鸟算鸟，都不容易~");
                    update_log("给你下一个吧");
                }

                update_log("我瞅瞅，现在那个臭XHC更新到那个版本了");
                Thread.Sleep(1000);
                ...
                else
                {
                    ...
                    {
                        update_log("什么，你这么幸运，竟然不用下载");
                    }
                    else
                    {
                        update_log($"我又可以帮助你了o(*￣▽￣*)ブ，你现在的版本:{localTag}喵 某个人更新的版本:{tag}喵，需要更新喵");
                    }
                }
                step.Current = 1;
                ...
                // 7. 解压文件（合并替换模式）
                step.Current = 3;
                update_log("这么大个压缩包，送给你喽！");
                update_log($"算了，给你解压吧喵 {extractPath}");
                ...
                try
                {
                    ...
                    update_log($"我要给你写一个新的版本号{tag} 喵");
                }
                catch (Exception ex)
                {
                    update_log($"失败了 {ex.Message} 喵，呜呜呜");
                }

                // 9. 启动程序
                step.Current = 4;
                update_log("我找找给你开哪里喵~");
                update_log("对了是app.exe喵~");
                update_log("打开了喵~");
                Thread.Sleep(1000);
                update_log("再见主人喵~");
                Thread.Sleep(1000);
            ...
            {
                update_log($"叕出错误了喵：{ex.Message}");
                update_log($"如果是病毒，请关闭杀毒软件喵");
                update_log($"复制完整的日志反馈吧喵：{ex.StackTrace}");
        ...
        private void LaunchBrowserWindow()
        {
            try
            {
                ...
                this.Invoke((MethodInvoker)delegate {
                    ...
                    browserForm.Show();
                    update_log("browser哥哥来了喵喵");
                });
            }
            catch (Exception ex)
            {
                update_log($"browser哥哥生气了喵 {ex.Message}");
            }
        }
        ...
        private void KillAppProcess()
        {
            try
            {
                ...
                {
                    update_log($"找到 {processes.Length} 个app.exe进程，正在终止喵~");
            ...
            {
                update_log($"app.exe又出错了喵 {ex.Message}");
            }
        }
        ...
        private async Task DownloadWithResumeAsync(string url, string filePath, int maxRetries, TimeSpan retryDelay)
        {
            ...
            string originalUrl = url; // 保存原始URL，用于重试时切换到镜像
            bool isUsingMirror = false; // 标记是否正在使用镜像
            const long MINIMUM_SPEED_THRESHOLD = 1024 * 10; // 10KB/s的最小速度阈值
            const int SPEED_CHECK_INTERVAL = 5000; // 5秒检查一次速度
            var speedCheckToken = new CancellationTokenSource();

            // 检查本地是否已存在部分下载的文件
            if (File.Exists(filePath))
            {
                alreadyDownloadedBytes = new FileInfo(filePath).Length;
                update_log($"嘿咻: {FormatFileSize(alreadyDownloadedBytes)}");
            }
            ...
            for (int attempt = 0; attempt <= maxRetries; attempt++)
            {
                ...
                {
                    // 在重试时检查是否应该切换到镜像URL
                    if (attempt > 0 && !isUsingMirror && originalUrl.StartsWith("https://github.com/"))
                    {
                        url = originalUrl.Replace("https://github.com/", MIRROR_PREFIX + "https://github.com/");
                        isUsingMirror = true;
                        update_log("主下载地址失败，切换到镜像地址重试喵~");
                    }
                    ...
                    using (var request = new HttpRequestMessage(HttpMethod.Get, url))
                    {
                        ...
                        if (alreadyDownloadedBytes > 0)
                        {
                            ...
                            update_log($"完了断了，赶紧记上位置 {alreadyDownloadedBytes}-");
                        }

                        update_log($"我试试喵 {attempt + 1}/{maxRetries + 1})喵~");
                        using (var response = await httpClient.SendAsync(request, HttpCompletionOption.ResponseHeadersRead))
                        {
                            ...
                            {
                                // 全新下载
                                update_log("服务器响应 200 OK喵，进行全新下载喵");
                            ...
                            if (canReportProgress)
                            {
                                ...
                                update_log($"文件太大了吧: {FormatFileSize(effectiveTotalBytes)}, 需要下载: {FormatFileSize(response.Content.Headers.ContentLength ?? 0)}");
                            }
                            else
                            {
                                update_log("完了文件大的我都看不见了");
                            }
                            ...
                            using (var fileStream = new FileStream(filePath, FileMode.Append, FileAccess.Write, FileShare.None, 8192, true)) // 使用 Append 模式
                            {
                                ...
                                int bytesRead;
                                var lastBytesReadTime = DateTime.Now;
                                var lastBytesReadCount = alreadyDownloadedBytes;
                                var isSpeedCheckRunning = true;

                                update_log("文件片来了，嘿咻，我要干活了，没空和你聊天了");

                                // 启动速度检测任务
                                _ = Task.Run(async () =>
                                {
                                    while (isSpeedCheckRunning && !speedCheckToken.Token.IsCancellationRequested)
                                    {
                                        await Task.Delay(SPEED_CHECK_INTERVAL, speedCheckToken.Token);
                                        
                                        // 检查下载速度，如果速度过慢且未使用镜像，则切换到镜像
                                        if (!isUsingMirror && originalUrl.StartsWith("https://github.com/"))
                                        {
                                            var timeElapsed = DateTime.Now.Subtract(lastBytesReadTime).TotalSeconds;
                                            if (timeElapsed > 0)
                                            {
                                                var speed = (alreadyDownloadedBytes - lastBytesReadCount) / timeElapsed;
                                                if (speed < MINIMUM_SPEED_THRESHOLD)
                                                {
                                                    update_log($"下载速度太慢了: {FormatFileSize((long)speed)}/s，下次尝试将使用镜像地址喵~");
                                                    // 我们不能直接在这里修改url，因为这需要重新发起请求
                                                    // 但我们可以设置一个标志，在下一次重试时使用镜像
                                                }
                                            }
                                        }
                                        lastBytesReadCount = alreadyDownloadedBytes;
                                        lastBytesReadTime = DateTime.Now;
                                    }
                                }, speedCheckToken.Token);
                                    ...
                                    while ((bytesRead = await contentStream.ReadAsync(buffer, 0, buffer.Length)) > 0)
                                    {
                                        ...
                                        if (canReportProgress)
                                        {
                                            ...
                                            if (currentProgress > lastReportedProgress)
                                            {
                                                ...
                                                update_log($"系统播报，下载进度: {displayProgress:P0} ({FormatFileSize(alreadyDownloadedBytes)}/{FormatFileSize(effectiveTotalBytes)})");
                                            }
                                            // 更新UI进度条 (0.1 到 0.6)
                                ...
                                {
                                    // 停止速度检测
                                    isSpeedCheckRunning = false;
                                    speedCheckToken.Cancel();
                                }

                                update_log($"真的吗成功了耶: {FormatFileSize(totalBytesReadInThisAttempt)} 已接收");
                                // 如果成功完成，跳出重试循环
                                return;
                            }
                        }
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        update_log($"失败了，呜呜呜喵 我再试试喵 {attempt + 1}/{maxRetries + 1}): {ex.Message}. 等待 {retryDelay.TotalSeconds} 秒后重试喵~");
        ...
        private async Task<T> RetryOnExceptionAsync<T>(Func<Task<T>> operation, int maxRetries, TimeSpan retryDelay)
        {
            for (int attempt = 0; attempt <= maxRetries; attempt++)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    {
                        update_log($"操作失败 (尝试 {attempt + 1}/{maxRetries + 1}): {ex.Message}. 等待 {retryDelay.TotalSeconds} 秒后重试喵~");
                        await Task.Delay(retryDelay);
                        // 可以在这里增加指数退避，例如 retryDelay *= 2;
                    }
                }
        ...
        private void ExtractFileWithMerge(string zipFilePath, string extractPath)
        {
            try
            {
                ...
                using (var archive = System.IO.Compression.ZipFile.OpenRead(zipFilePath))
                {
                    ...
                    update_log($"ZIP文件包含 {totalEntries} 个项目喵~");
                ...
                try
                {
                    ...
                    update_log($"临时文件已清理喵~: {Path.GetFileName(zipFilePath)}");
                }
                catch (Exception ex)
                {
                    update_log($"临时文件清理失败喵~: {ex.Message}");
                }
            }
            catch (Exception ex)
            {
                throw new Exception($"解压文件失败喵~: {ex.Message}");
            }
        }
